...
;; lets add this -- I can use this all over the plac for the "add-subclass"
;; type functions.
...
(print-a-lot "x" '("y"))
(print-a-lot "x" 1 2 3)
(print-a-lot "x" 1 2 [3 4 5 [6 7 8]])


(defn ontology-first-maybe [f]
  (fn [& args]
    (if (instance? OWLOntology
         (first args))
      (with-ontology (first args)
        (apply f (rest args)))
      (apply f args))))

(def ontology-vectorize
  (comp ontology-first-maybe vectorize))
